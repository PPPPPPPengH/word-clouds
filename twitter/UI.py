...
from word_cloud import get_text_tweets, create_wordcloud
...
class main_ui(QMainWindow):
    ...
    def dialog_unvalid_date(self):
        msg = QtWidgets.QMessageBox()
        msg.setIcon(QtWidgets.QMessageBox.Warning)
        msg.setText("Date is non valid.")
        msg.setWindowTitle("Unvalid date")
        msg.setStandardButtons(QtWidgets.QMessageBox.Ok)    
        retval = msg.exec_()

    def dialog_emtpy_search(self):
        msg = QtWidgets.QMessageBox()
        msg.setIcon(QtWidgets.QMessageBox.Warning)
        msg.setText("The search box is empty!")
        msg.setWindowTitle("Unable to search")
        msg.setStandardButtons(QtWidgets.QMessageBox.Ok)    
        retval = msg.exec_()
    
    def dialog_search_completed(self):
        msg = QtWidgets.QMessageBox()
        msg.setIcon(QtWidgets.QMessageBox.Information)
        msg.setText("WordCloud created successfully!")
    ...
    def search_tweets(self):
        ...
        if compare_dates(day_start,month_start,year_start,day_end,month_end,year_end)==False:
            ...
            if searched_word == "":
                self.dialog_emtpy_search()
                return False
            else:
                dates_string = dates_to_string(day_start, month_start, year_start, day_end, month_end, year_end)
                lan = self.pick_lang.currentText()
                tweets = get_tweets(lan = lan, search = searched_word, start_time=dates_string[0], end_time=dates_string[1])
                
                tweets_text(tweets, searched_word=searched_word,lan=lan)

                tweets_loaded = get_text_tweets(searched_word, lan)
                create_wordcloud(lan, tweets_loaded, name=searched_word,initial_month=str(month_start),initial_year=str(year_start),final_month=str(month_end),final_year=str(year_end))



                self.dialog_search_completed()
                return True
